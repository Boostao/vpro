...
test_that("climate data sensibility checks (if actual data available)", {
  ...
  expect_true(climate$MAT > 5 && climate$MAT < 12, info = sprintf("MAT out of expected range: %.2f", climate$MAT))

  expect_true(climate$MAP > 200 && climate$MAP < 400, info = sprintf("MAP out of expected range: %.2f", climate$MAP))

  expect_true(climate$MWMT > 14 && climate$MWMT < 22, info = sprintf("MWMT out of expected range: %.2f", climate$MWMT))

  expect_true(climate$MCMT > -5 && climate$MCMT < 2, info = sprintf("MCMT out of expected range: %.2f", climate$MCMT))
})
