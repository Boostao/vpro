...
test_that("test schema initializes with required tables", {
  ...
  for (tbl in expected_tables) {
    expect_true(tbl %in% core_tables$table_name, label = paste0("Table 'core.", tbl, "' exists"))
  }

  # Check for lists schema tables
  lists_tables <- DBI::dbGetQuery(
    con,
    "
    SELECT table_name FROM information_schema.tables 
...
test_that("insert_test_plot() creates valid plot records", {
  ...
  insert_test_plot(con, plot_number = "TEST-PLOT-01", species = c("AB", "FD", "HW"), cover_percents = c(30, 40, 20))

  # Verify environment record
  env_result <- DBI::dbGetQuery(con, "SELECT COUNT(*) as n FROM core.env WHERE plot_number = 'TEST-PLOT-01'")
  expect_equal(env_result$n[1], 1L)

  # Verify vegetation records
  veg_result <- DBI::dbGetQuery(con, "SELECT COUNT(*) as n FROM core.veg WHERE plot_number = 'TEST-PLOT-01'")
...
test_that("Data validation constraints are enforced", {
  ...
  DBI::dbAppendTable(con, DBI::Id(schema = "core", table = "env"), invalid_env)

  # Verify the record was inserted
  result <- DBI::dbGetQuery(con, "SELECT COUNT(*) as n FROM core.env WHERE plot_number = 'INVALID-01'")
  ...
  tryCatch(
    {
      ...
      insert_test_plot(con, plot_number = "CLOUD-TEST-01", species = c("AB"), cover_percents = c(25))

      # Verify local record was created
      local_count <- DBI::dbGetQuery(
        con,
        "
