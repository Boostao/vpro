...
if (FALSE) {
  ...
  count_conflicts <- function(project_id = NULL) {
    ...
    if (is.null(project_id)) {
      ...
      result <- dbGetQuery(con, "SELECT COUNT(*) AS n FROM sync_conflicts WHERE project_id = ?", list(project_id))
    }
  ...
  verify_conflict_resolved <- function(conflict_id) {
    ...
    result <- dbGetQuery(con, "SELECT COUNT(*) AS n FROM sync_conflicts WHERE id = ?", list(conflict_id))
  ...
  create_plot_for_conflict_test <- function(plot_number, project_id = "TEST") {
    ...
    tryCatch(
      {
        dbExecute(con, "INSERT INTO Sample_SU (SiteUnit, ProjectID) VALUES (?, ?)", list(plot_number, project_id))
      },
  ...
  test_that("sync infrastructure tables are created correctly", {
    skip_if_not(file.exists("data/vpro.duckdb"), message = "Main database not found - run scripts/01_build_database.R first")
    ...
    expect_true("sync_state" %in% tables, info = "sync_state table should exist")

    # Verify sync_conflicts table exists
    expect_true("sync_conflicts" %in% tables, info = "sync_conflicts table should exist")

    # Verify sync_conflicts has correct columns
    conflict_cols <- dbListFields(con, "sync_conflicts")
    expect_true(all(c("id", "table_name", "plot_number", "project_id", "details") %in% conflict_cols), info = "sync_conflicts should have all required columns")
  })
  ...
  test_that("conflict creation helper works correctly", {
    ...
    expect_true(is.numeric(conflict_id) && conflict_id > 0, info = "Conflict creation should return valid ID")

    # Verify conflict was created
    count <- count_conflicts(project_id = "DEMO")
    expect_equal(count, 1, info = "Should have exactly 1 conflict")
  ...
  test_that("multiple conflicts can be created and counted", {
    ...
    expect_length(conflict_ids, 5, info = "Should create 5 conflicts")

    total_count <- count_conflicts()
    expect_equal(total_count, 5, info = "Database should contain 5 conflicts")

    # Count by project
    test_count <- count_conflicts(project_id = "TEST")
    expect_equal(test_count, 5, info = "All test conflicts have TEST project_id")
  ...
  test_that("conflict resolution UI appears when conflicts exist", {
    ...
    expect_equal(conflict_badge, "3", info = "Should display conflict count")
    ...
    expect_true(!is.null(conflicts_table), info = "Conflict list table should be populated")
  ...
  test_that("single field conflict shows diff viewer", {
    ...
    expect_true(grepl("sparse vegetation", diff_display$local, fixed = TRUE), info = "Local value should be displayed")
    expect_true(grepl("dense canopy", diff_display$cloud, fixed = TRUE), info = "Cloud value should be displayed")

    # Verify field name is highlighted
    expect_true(grepl("SiteNotes", diff_display$field_name), info = "Field name should be shown")
  ...
  test_that("user can choose 'Keep Local' resolution", {
    ...
    app$expect_values(output = "admin-resolution_status", value = "✓ Conflict resolved (kept local)")

    # Verify conflict was removed from database
    expect_true(verify_conflict_resolved(conflict_id), info = "Conflict should be deleted after resolution")
  ...
  test_that("user can choose 'Keep Cloud' resolution", {
    ...
    # Verify resolution
    expect_true(verify_conflict_resolved(conflict_id), info = "Conflict should be resolved")
  ...
  test_that("user can dismiss conflicts", {
    ...
    expect_true(verify_conflict_resolved(conflict_id), info = "Dismissed conflict should be removed from list")
  ...
  test_that("multiple conflicts can be displayed in list", {
    ...
    expect_equal(nrow(conflict_table), 5, info = "Should display all 5 conflicts")

    # Verify table columns
    expect_true(
      all(
        c("plot_number", "table_name", "detected_utc") %in%
  ...
  test_that("Accept All Local batch action works", {
    ...
    expect_equal(remaining, 0, info = "All conflicts should be resolved")
  ...
  test_that("selective resolution from list works", {
    ...
    remaining <- count_conflicts(project_id = "TEST")
    expect_equal(remaining, 2, info = "Should have 2 unresolved conflicts")
  ...
  test_that("conflict on deleted record shows special UI", {
    ...
    expect_true(grepl("DELETED LOCALLY", diff_view$status, ignore.case = TRUE), info = "Should indicate deletion conflict")

    # Verify options: "Restore Local" vs "Keep Cloud Edit"
    button_labels <- app$get_value(output = "admin-resolution_buttons")
    expect_true("restore_local" %in% names(button_labels), info = "Should offer restore option")
  ...
  test_that("sync cancellation preserves local data", {
    ...
    db_path <- if (file.exists("data/vpro.duckdb")) "data/vpro.duckdb" else ":memory:"
    con <- dbConnect(duckdb(), db_path)
    initial_data <- dbGetQuery(con, "SELECT * FROM Sample_Env WHERE PlotNumber = 'CANCEL-001'")
    dbDisconnect(con)
    ...
    db_path <- if (file.exists("data/vpro.duckdb")) "data/vpro.duckdb" else ":memory:"
    con <- dbConnect(duckdb(), db_path)
    final_data <- dbGetQuery(con, "SELECT * FROM Sample_Env WHERE PlotNumber = 'CANCEL-001'")
    dbDisconnect(con)

    expect_equal(initial_data, final_data, info = "Cancellation should not modify local data")

    # Verify conflicts still exist
    remaining <- count_conflicts(project_id = "TEST")
    expect_equal(remaining, 2, info = "Conflicts should remain after cancellation")
  ...
  test_that("re-sync after partial resolution works correctly", {
    ...
    expect_equal(count_conflicts(project_id = "TEST"), 2, info = "Should have 2 unresolved conflicts")
    ...
    expect_gte(final_count, 2, info = "Unresolved conflicts should persist")
  ...
  test_that("conflict UI is hidden when no conflicts exist", {
    ...
    conflict_badge <- app$get_value(output = "admin-conflict_count")
    expect_true(is.null(conflict_badge) || conflict_badge == "0", info = "Should show no conflicts")

    # Verify "Resolve Conflicts" button is disabled or hidden
    button_state <- app$get_value(input = "admin-resolve_conflicts")
    expect_true(is.null(button_state) || !button_state$enabled, info = "Resolve button should be disabled when no conflicts")
  ...
  test_that("conflict list paginates for large conflict sets", {
    ...
    expect_true(!is.null(pagination), info = "Pagination should appear for large conflict lists")

    # Verify initial page shows 25 conflicts (default page size)
    conflict_table <- app$get_value(output = "admin-conflict_list")
    expect_lte(nrow(conflict_table), 25, info = "Should limit initial display to page size")
  ...
  test_that("conflict resolution completes within reasonable time", {
    ...
    expect_lt(elapsed, 10, info = paste("Batch resolution took", round(elapsed, 1), "seconds - should be < 10"))
