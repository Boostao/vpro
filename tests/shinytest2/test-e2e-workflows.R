...
test_that("E2E Workflow 1: Project selection loads plots and context", {
  ...
  if (!is.null(plot) && nzchar(plot)) {
    ...
    result <- verify_db_state(
      sprintf("SELECT * FROM Sample_Env WHERE ProjectID = '%s' AND PlotNumber = '%s'", project, plot),
...
test_that("E2E Data Integrity: Referential integrity enforcement", {
  ...
  expect_equal(length(invalid_projects), 0, label = "All Sample_Env projects exist in Sample_Metadata")
})
...
test_that("E2E State Persistence: Context persists across tab navigation", {
  ...
  for (tab in tabs) {
    ...
    expect_equal(current_project, project, label = paste0("Project persists on ", tab, " tab"))
    expect_equal(current_plot, plot, label = paste0("Plot persists on ", tab, " tab"))
  }
...
test_that("E2E Performance: Rapid plot switching loads correctly", {
  ...
  plots_query <- verify_db_state(
    sprintf("SELECT DISTINCT PlotNumber FROM Sample_Env WHERE ProjectID = '%s' ORDER BY PlotNumber LIMIT 5", project),
  ...
  for (i in seq_len(min(3, nrow(plots_query)))) {
    ...
    current_plot <- safe_get_value(app, input = "sel_su")
    expect_equal(current_plot, plot_id, label = paste0("Plot switch ", i, " successful"))

    # Verify vegetation table still renders
    veg_hot <- safe_get_value(app, output = "veg-hot_veg_a")
    expect_true(!is.null(veg_hot), label = paste0("Vegetation renders after switch ", i))
  }
})
...
test_that("E2E Database: Export format consistency", {
  ...
  for (combo in combinations) {
    ...
    expect_true(!is.null(csv_btn), label = sprintf("CSV available (L=%s, P=%s)", combo$lumping, combo$pivot))
    expect_true(!is.null(rds_btn), label = sprintf("RDS available (L=%s, P=%s)", combo$lumping, combo$pivot))
  }
})
