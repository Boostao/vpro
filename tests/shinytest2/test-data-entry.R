...
describe("Vegetation Data Entry - Layer Navigation", {
  test_that("Layer switching preserves tab context", {
    ...
    for (layer in layers) {
      ...
      expect_equal(current_layer, layer, label = paste("Navigate to", layer))
    }

    # Return to first layer
    final_layer <- navigate_to_veg_layer(app, "Layer A (Trees)")
    expect_equal(final_layer, "Layer A (Trees)", label = "Return to Layer A")
  })
  ...
  test_that("Layer tabs render handsontable for each layer", {
    ...
    for (i in seq_along(hot_outputs)) {
      ...
      expect_true(hot_exists, label = paste("Handsontable exists for", layer_names[i]))
    }
  })
  ...
  test_that("Vegetation context hint updates with current plot", {
    ...
    expect_true(!is.null(context_hint), label = "Context hint renders for current plot")
  })
})
...
describe("Vegetation Data Entry - Add/Delete Operations", {
  test_that("Add Species button exists on all layers", {
    ...
    for (layer in layers) {
      ...
      expect_true(add_btn_exists, label = paste("Add Species button exists on", layer))
    }
  })
  ...
  test_that("Delete Selected button exists on all layers", {
    ...
    for (layer in layers) {
      ...
      expect_true(del_btn_exists, label = paste("Delete button exists on", layer))
    }
  })
...
describe("Site & Environment - Coordinate Calculations", {
  test_that("Latitude field accepts decimal degrees", {
    ...
    expect_equal(lat_value, "49.2827", label = "Latitude accepts decimal degrees")
  })
  ...
  test_that("Longitude field accepts decimal degrees", {
    ...
    expect_equal(lon_value, "-123.1207", label = "Longitude accepts decimal degrees")
  })
...
describe("Site & Environment - Field Validation", {
  test_that("Elevation accepts numeric values", {
    ...
    expect_equal(elev_value, 1200, label = "Elevation accepts numeric input")
  })
  ...
  test_that("Aspect accepts 0-360 degrees", {
    ...
    expect_equal(aspect_value, 180, label = "Aspect accepts degree value")
  })
  ...
  test_that("Date picker enforces valid dates", {
    ...
    expect_equal(date_value, test_date, label = "Date picker accepts valid date")
  })
})
...
describe("Site & Environment - Dropdowns & Code Lists", {
  test_that("Moisture regime dropdown populates from code lists", {
    ...
    expect_true(moisture_exists, label = "Moisture regime dropdown exists")
  })
  ...
  test_that("Nutrient regime dropdown populates from code lists", {
    ...
    expect_true(nutrient_exists, label = "Nutrient regime dropdown exists")
  })
  ...
  test_that("Meso slope position dropdown exists", {
    ...
    expect_true(meso_exists, label = "Meso slope position dropdown exists")
  })
})
...
describe("Data Entry Edge Cases", {
  ...
  test_that("Empty/NULL values handle gracefully", {
    ...
    expect_true(!is.null(location_field), label = "App responsive after empty numeric field")
  })
