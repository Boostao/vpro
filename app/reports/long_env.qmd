...
source(file.path(root_dir, "reports", "_vpro_report_utils.R"))
...
if (!is.null(parquet_dir) && nzchar(trimws(parquet_dir))) {
  ...
  if (file.exists(lists_db)) {
    dbExecute(
      con,
      sprintf("ATTACH '%s' AS lists", gsub("'", "''", lists_db))
    )
  }
}
```
...
if (nrow(env) > 0) {
  if ("siteunit" %in% names(env)) {
    ...
    for (name in names(split_env)) {
      ...
      apply_table_theme <- apply_theme && has_kableExtra
      ...
      if (apply_table_theme && nrow(section) > 0) {
        style_numeric <- function(vec) {
          ...
          vapply(seq_along(labels), function(i) {
            ...
            if (nzchar(bg) && has_kableExtra) kableExtra::cell_spec(label, background = bg) else label
          }, character(1))
        }
      ...
      tbl <- kbl(section, align = "l", escape = !apply_table_theme)
      if (apply_table_theme) {
    ...
    colour_greater <- suppressWarnings(as.numeric(params$colour_greater))
    gray_greater <- suppressWarnings(as.numeric(params$gray_greater))
    if (apply_table_theme && nrow(env) > 0) {
      style_numeric <- function(vec) {
        labels <- as.character(vec)
        vapply(seq_along(labels), function(i) {
          label <- labels[[i]]
    ...
    tbl <- kbl(env, align = "l", escape = !apply_table_theme)
    if (apply_table_theme) {
      tbl <- kableExtra::kable_styling(
        tbl,
        bootstrap_options = c("striped", "hover", "condensed"),
