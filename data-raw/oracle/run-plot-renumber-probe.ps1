param(
  ...
  [string]$ModulePath,
  [Parameter(Mandatory = $true)]
  [ValidateSet("form-success", "form-cancel", "form-collision", "dao-success", "dao-collision")]
  [string]$Mode,
  [Parameter(Mandatory = $true)]
...
$oracleRoot = Join-Path $env:LOCALAPPDATA "Temp\vpro-plot-renumber-$Mode-$stamp"
...
$output = Join-Path $oracleRoot "plot-renumber-$Mode.tsv"
...
try {
  ...
  $access.LoadFromText(5, "modPlotRenumberProbe", $ModulePath)
  $escapedOutput = $output.Replace('"', '""')
  $escapedMode = $Mode.Replace('"', '""')
  $escapedPlot = $NewPlot.Replace('"', '""')
  $returned = $access.Eval("RunPlotRenumberOracle(""$escapedOutput"",""$escapedMode"",""$escapedPlot"")")
