save_site_env_header <- function(con, plot_id, fields, project_id = NULL, user = NULL) {
  ...
  if (res == 0) {
    sql_ins <- paste(
      "INSERT INTO",
      env_table_sql,
      "(plotnumber, _location, date, sitesurveyor, latitude,",
  ...
  if (nrow(existing) > 0) {
    ...
    for (field_name in names(fields)) {
      if (!(field_name %in% names(old_row))) {
        next
      }
...
get_hot_selected_row <- function(selection) {
  ...
  if (is.list(selection) && !is.null(selection$r)) {
    return(selection$r)
  }
  if (is.list(selection) && !is.null(selection$select) && !is.null(selection$select$r)) {
    return(selection$select$r)
  }
  if (is.matrix(selection) && ncol(selection) >= 1) {
  ...
  tagList(
    card(
      ...
      navset_card_tab(
        nav_panel(
          ...
          div(
            ...
            div(
              ...
              layout_columns(
                div(
                  actionButton(ns("btn_fetch_climate"), "Fetch Climate Data", class = "btn-info", icon = icon("cloud-download")),
                  checkboxInput(ns("chk_auto_fetch_climate"), "Auto-fetch on coordinate change", value = FALSE),
          ...
          hr(),
          div(class = "d-flex align-items-center gap-2", actionButton(ns("run_compliance"), "Run Compliance", class = "btn-secondary"), textOutput(ns("compliance_status"))),
        ...
        nav_panel("Audit", selectInput(ns("env_audit_table"), "Table", choices = c("All" = "", "Env", "Humus", "Mineral")), DT::DTOutput(ns("dt_audit_env")))
      )
    )
  )
}
...
mod_site_env_server <- function(id, sys_state, con) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$btn_fetch_climate, {
      ...
      show_toast(toast("Fetching climate data from ClimR...", id = "climr_fetch", type = "success", duration_s = NA))
    ...
    observeEvent(
      list(input$env_lat, input$env_long),
      {
        # Only auto-fetch if checkbox is enabled
        if (!isTRUE(input$chk_auto_fetch_climate)) {
          return()
        }
    ...
    output$climr_status <- renderUI({
      ...
      if (!is_available) {
        return(
          div(
            ...
            p(class = "mb-0 small", "Install with: ", code("remotes::install_github('bcgov/climr')")),
      ...
      p(class = "text-success small", icon("check-circle"), sprintf(" Last fetch: %s", fetch_time))
    })
    ...
    output$climr_summary <- renderUI({
      ...
      if (is.na(climate$MAT)) {
        return(
          div(class = "alert alert-info", p(class = "mb-0", "Climate data structure ready, but ClimR package needs full configuration."))
        )
      }
      ...
      tagList(
        tags$table(
          ...
          tags$tbody(
            tags$tr(
              tags$td(strong("MAT:")),
              tags$td(sprintf("%.1f°C", climate$MAT)),
              tags$td(strong("MAP:")),
              tags$td(sprintf("%.0f mm", climate$MAP))
            ),
            tags$tr(
              tags$td(strong("MWMT:")),
              tags$td(sprintf("%.1f°C", climate$MWMT)),
              tags$td(strong("MCMT:")),
              tags$td(sprintf("%.1f°C", climate$MCMT))
            ),
            tags$tr(
              tags$td(strong("AHM:")),
              tags$td(sprintf("%.1f", climate$AHM)),
              tags$td(strong("NFFD:")),
              tags$td(sprintf("%.0f days", climate$NFFD))
            ),
            tags$tr(
              tags$td(strong("Elevation:")),
              tags$td(sprintf("%.0f m", climate$elevation)),
              tags$td(strong("Period:")),
              tags$td(climate$period)
            )
          )
        )
      )
    ...
    update_soil_from_hot <- function(hot_input, table, display_cols) {
      ...
      for (row_idx in seq_len(nrow(new_df))) {
        for (col_name in valid_cols) {
          ...
          if (!is.na(old_val) && !is.na(new_val) && as.character(old_val) == as.character(new_val)) {
            next
          }
          if (is.na(old_val) && nzchar(as.character(new_val)) == FALSE) {
            next
          }
    ...
    modal_humus <- function(new, record = NULL) {
      ...
      modalDialog(
        ...
        layout_columns(
          ...
          if (!is.null(c_horizon)) {
            selectInput(ns("h_horizon"), "Horizon", choices = c("", c_horizon), selected = if (!new) record$horizon else "")
          } else {
            textInput(ns("h_horizon"), "Horizon", value = if (!new) record$horizon else "")
          },
        ...
        layout_columns(
          if (!is.null(c_deg)) {
            selectInput(ns("h_deg"), "Structure Degree", choices = c("", c_deg), selected = if (!new) record$humusstructuredegree else "")
          } else {
            textInput(ns("h_deg"), "Structure Degree", value = if (!new) record$humusstructuredegree else "")
          },

          if (!is.null(c_kind)) {
            selectInput(ns("h_kind"), "Structure Kind", choices = c("", c_kind), selected = if (!new) record$humusstructurekind else "")
          } else {
            textInput(ns("h_kind"), "Structure Kind", value = if (!new) record$humusstructurekind else "")
          },
    ...
    save_soil_record <- function(table, mode, id, fields) {
      ...
      if (mode == "new") {
        ...
        tryCatch(
          ...
          error = function(e) {
            show_toast(toast(paste("Insert Error:", e$message), type = "danger"))
          }
        )
      } else {
        table_id <- db_id(table, config("Current", "CurrProject"), prj = TRUE)
