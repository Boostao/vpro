...
mod_admin_codes_server <- function(id, state, con) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$code_list_select, {
      ...
      df <- dbGetQuery(con, "SELECT item, itemdescription, itemorder FROM VLists.USysTableOfLists WHERE listname = ? ORDER BY itemorder", list(input$code_list_select))
      rv_codes$data <- df
    ...
    output$code_dt <- renderDT({
      req(rv_codes$data)
      datatable(rv_codes$data, editable = 'cell', selection = 'none', options = list(pageLength = 15, dom = 't,p'))
    })
    ...
    observeEvent(input$code_save, {
      ...
      old_rows <- dbGetQuery(con, "SELECT item, itemdescription, itemorder FROM VLists.USysTableOfLists WHERE listname = ?", list(lname))
      dbBegin(con)
      tryCatch(
        {
