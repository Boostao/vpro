...
mod_admin_master_server <- function(id, state, con) {
  moduleServer(id, function(input, output, session) {
    ...
    output$master_dt <- renderDT({
      ...
      datatable(rv_master$data, editable = list(target = "cell", disable = disable_cols), selection = "none", options = list(pageLength = 15, dom = "t,p"))
    })
    ...
    observeEvent(input$master_add_row, {
      ...
      if (!require_permission(c("manage:codes", "write:all"), "Permission required: manage codes")) {
        return()
      }
      new_row <- as.list(rep(NA, length(rv_master$columns)))
      names(new_row) <- rv_master$columns
      if ("SiteSeries" %in% rv_master$columns) {
        new_row$SiteSeries <- "NEW_UNIT"
      }
      if ("SiteSeriesLongName" %in% rv_master$columns) {
        new_row$SiteSeriesLongName <- "New Site Unit"
      }
      if ("SiteSeriesScientificName" %in% rv_master$columns) {
        new_row$SiteSeriesScientificName <- ""
      }
      if ("Level" %in% rv_master$columns) {
        new_row$Level <- 11
      }
      if ("ID" %in% rv_master$columns) {
