# FS882-8x6xl Ecosystem Field Form Module (8-column x 6-row layout variant)
# Reuses FS882-6x4 server logic (identical data model, only UI layout differs)
# Migrated from Access FS882-8x6XL form
...
mod_fs882_8x6xl_ui <- function(id) {
  ns <- NS(id)
  ...
  card(
    ...
    card_header(
      ...
      div(
        ...
        actionButton(ns("btnNavFirst"), NULL, icon = icon("backward-step"), class = "btn btn-outline-secondary btn-sm px-1"),
        actionButton(ns("btnNavPrev"), NULL, icon = icon("caret-left"), class = "btn btn-outline-secondary btn-sm px-1"),
        div(
          class = "d-flex flex-column align-items-center",
          style = "min-width: 160px;",
          selectizeInput(ns("navPlotPicker"), NULL, choices = NULL, width = "160px", options = list(placeholder = "Plot...")),
          tags$small(class = "text-body-secondary", textOutput(ns("navRecordCount"), inline = TRUE))
        ),
        actionButton(ns("btnNavNext"), NULL, icon = icon("caret-right"), class = "btn btn-outline-secondary btn-sm px-1"),
        actionButton(ns("btnNavLast"), NULL, icon = icon("forward-step"), class = "btn btn-outline-secondary btn-sm px-1"),
        actionButton(ns("btnNavNew"), NULL, icon = icon("plus"), class = "btn btn-outline-success btn-sm px-1", title = "New record"),
        ...
        div(
          ...
          tags$button(class = "btn btn-outline-secondary btn-sm", type = "button", id = ns("btnNavSearch"), icon("magnifying-glass"))
        ),
        tags$div(class = "vr mx-1"),
        # -- Tool buttons --
          ...
          card(
            ...
            card_body(
              ...
              div(
                ...
                tags$span(class = "fw-semibold text-nowrap form-label-sm", style = "width:7rem;padding-bottom:.35rem;", "Data Quality"),
                div(style = "flex:0 0 auto;width:7.5rem;", selectInput(ns("SitePlotQuality"), "Site", choices = NULL, width = "100%")),
                div(style = "flex:0 0 auto;width:7.5rem;", selectInput(ns("VegPlotQuality"), "Veg", choices = NULL, width = "100%")),
                div(style = "flex:0 0 auto;width:7.5rem;", selectInput(ns("SoilPlotQuality"), "Soil", choices = NULL, width = "100%"))
              ),
              # Exposure Type — Access: "Exposure Type" label + Exposure1/Exposure2
              div(
                class = "d-flex align-items-end gap-3 mb-2",
                tags$span(class = "fw-semibold text-nowrap form-label-sm", style = "width:7rem;padding-bottom:.35rem;", "Exposure Type"),
                div(style = "flex:0 0 auto;width:7.5rem;", selectInput(ns("Exposure1"), NULL, choices = NULL, width = "100%")),
                div(style = "flex:0 0 auto;width:7.5rem;", selectInput(ns("Exposure2"), NULL, choices = NULL, width = "100%"))
              ),
              # Site Disturbance — Access: "Site Disturbance" label + 3 dropdowns
              div(
                class = "d-flex align-items-end gap-3",
                tags$span(class = "fw-semibold text-nowrap form-label-sm", style = "width:7rem;padding-bottom:.35rem;", "Site Disturbance"),
                div(style = "flex:0 0 auto;width:7.5rem;", selectInput(ns("SiteDisturbance1"), NULL, choices = NULL, width = "100%")),
                div(style = "flex:0 0 auto;width:7.5rem;", selectInput(ns("SiteDisturbance2"), NULL, choices = NULL, width = "100%")),
                div(style = "flex:0 0 auto;width:7.5rem;", selectInput(ns("SiteDisturbance3"), NULL, choices = NULL, width = "100%"))
              )
            )
          ),
...
mod_fs882_8x6xl_server <- function(id, state, con) {
      ...
      DT::renderDT(
        {
          ...
          col_labels <- if (grid_type == "A") {
            if (rv$cover_and_height) {
              ...
              c(species = "Tree/Shrubs", cover1 = "A1", cover2 = "A2", cover3 = "A3", totala = "A", cover4 = "B1", cover5 = "B2", totalb = "B", collected = "Coll")
            }
          } else if (grid_type == "C") {
            if (rv$cover_and_height) {
