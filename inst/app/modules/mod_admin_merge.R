...
mod_admin_merge_server <- function(id, state, con) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(
      list(state$AuthAuthenticated, state$AuthRole),
      {
      ...
      tryCatch(
        {
          ...
          rv$conflict_status <- if (nrow(rv$conflicts) == 0) {
            "No conflicts."
          } else {
    ...
    output$merge_conflict_status <- renderText({
      rv$conflict_status
    })
    ...
    output$merge_actions <- renderUI({
      ...
      tagList(
        actionButton(ns("merge_approve"), "Approve + Merge", class = if (disabled) "btn-secondary" else "btn-primary", disabled = disabled),
    ...
    output$merge_conflicts <- DT::renderDT({
      ...
      DT::datatable(rv$conflicts, rownames = FALSE, selection = "single", options = list(pageLength = 8, scrollX = TRUE))
    })
  })
}
