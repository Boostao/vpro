  ...
  tagList(
    card(
      ...
      navset_card_tab(
        ...
        nav_panel("Audit", selectInput(ns("veg_audit_table"), "Table", choices = c("Veg")), DT::DTOutput(ns("dt_audit_veg"))),
        nav_panel(
          "Compliance",
...
mod_veg_sample_server <- function(id, sys_state, con) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$hot_veg_a, {
      update_from_hot(input$hot_veg_a, display_cols_a)
    })
    observeEvent(input$hot_veg_b, {
      update_from_hot(input$hot_veg_b, display_cols_b)
    })
    observeEvent(input$hot_veg_c, {
      update_from_hot(input$hot_veg_c, display_cols_c)
    })
    observeEvent(input$hot_veg_d, {
      update_from_hot(input$hot_veg_d, display_cols_d)
    })
      ...
      tryCatch(
        {
          ...
          dbExecute(con, "INSERT INTO Veg (id, plotnumber, species) VALUES (?, ?, ?)", list(new_id, plot_id, new_species))
    ...
    observeEvent(input$btn_del_spp, {
      ...
      idx <- NULL
      if (input$layers_tab == "Layer A (Trees)") {
        idx <- get_hot_selected_row(input$hot_veg_a_select)
      } else if (input$layers_tab == "Layer B (Shrubs)") {
        idx <- get_hot_selected_row(input$hot_veg_b_select)
      } else if (input$layers_tab == "Layer C (Herbs)") {
        idx <- get_hot_selected_row(input$hot_veg_c_select)
      } else if (input$layers_tab == "Layer D (Moss)") {
        idx <- get_hot_selected_row(input$hot_veg_d_select)
      }
