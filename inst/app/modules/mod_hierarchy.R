...
set_lowest_tilde_levels <- function(con, table_name, tilde_ids) {
  ...
  if (!hierarchy_table_has_column(con, table_name, "Level")) {
    return(0L)
  }
...
create_lowest_breakpoints_table <- function(con, source_table = "Hierarchy", target_table = "USysLowestBreakpoints_Hierarchy") {
  ...
  if (!all(c("ID", "Name", "Parent") %in% select_cols)) {
    return(list(count = 0L, roots = 0L))
  }
...
sync_env_to_su <- function(con) {
  if (!table_has_columns(con, "Env", c("PlotNumber", "UserSiteUnit"))) {
    return(0L)
  }
  if (!table_has_columns(con, "SU", c("PlotNumber", "SiteUnit"))) {
    return(0L)
  }
...
copy_bec_to_su <- function(con) {
  if (!table_has_columns(con, "Env", c("PlotNumber", "BECSiteUnit"))) {
    return(0L)
  }
  if (!table_has_columns(con, "SU", c("PlotNumber", "SiteUnit"))) {
    return(0L)
...
get_env_distinct_values <- function(con, column, zone = NULL) {
  ...
  if (!(column %in% DBI::dbListFields(con, "Env"))) {
    return(character(0))
  }
...
build_su_from_env <- function(con, zone = NULL, subzone = NULL, replace = TRUE) {
  if (!table_has_columns(con, "Env", c("PlotNumber"))) {
    return(0L)
  }
  if (!table_has_columns(con, "SU", c("PlotNumber", "SiteUnit"))) {
    return(0L)
...
filter_duplicate_names <- function(source_df, existing_names) {
  if (nrow(source_df) == 0) {
    return(list(data = source_df, dropped = 0L))
  }
...
resolve_duplicate_names <- function(source_df, existing_names, mode = c("skip", "prefix", "suffix"), prefix = "", suffix = "") {
  if (nrow(source_df) == 0) {
    return(list(data = source_df, dropped = 0L, renamed = 0L))
  }
  if (length(existing_names) == 0) {
    return(list(data = source_df, dropped = 0L, renamed = 0L))
  }
  ...
  if (!any(dupes)) {
    return(list(data = updated, dropped = 0L, renamed = 0L))
  }
  ...
  for (old_id in order_ids) {
    ...
    if (use_order) {
      if ("MyOrder" %in% names(subtree) && !is.na(row$MyOrder)) {
        ...
        if (is.null(child_counters[[parent_key]])) {
          child_counters[[parent_key]] <- 0L
        }
...
insert_rekeyed_hierarchy <- function(con, source_df, parent_level = -1L) {
  if (nrow(source_df) == 0) {
    return(list(count = 0L, missing_parents = 0L))
  }
      ...
      navset_card_tab(
        ...
        nav_panel(
          ...
          tags$script(HTML(sprintf(
            ...
            ns("hier_find"),
            ns("hier_find_enter"),
            ns("hier_find_next_key"),
            ns("hier_find_prev_key")
          ))),
          verbatimTextOutput(ns("merge_preview")),
          div(style = "height:600px; overflow:auto;", shinyTree::shinyTree(ns("hier_tree"))),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    log_su_audit_changes <- function(before_df, after_df) {
      if (is.null(before_df)) {
        before_df <- data.frame(PlotNumber = character(0), SiteUnit = character(0), stringsAsFactors = FALSE)
      }
      if (is.null(after_df)) {
        after_df <- data.frame(PlotNumber = character(0), SiteUnit = character(0), stringsAsFactors = FALSE)
      }
    ...
    log_env_siteunit_changes <- function(before_df, after_df, field_name = "UserSiteUnit") {
      ...
      if (!(field_name %in% names(before_df)) || !(field_name %in% names(after_df))) {
        return(0L)
      }
      if (!("PlotNumber" %in% names(before_df)) || !("PlotNumber" %in% names(after_df))) {
        return(0L)
      }
    ...
    output$merge_preview <- renderText({
      ...
      if (nzchar(dup_preview)) {
        message <- paste(message, "| dupes:", dup_preview)
      }
    ...
    observeEvent(input$hier_view_plot_data, {
      ...
      if (DBI::dbExistsTable(con, "Env")) {
        ...
        project_col <- if ("projectid" %in% env_fields) {
          "projectid"
        } else if ("ProjectID" %in% env_fields) {
          "ProjectID"
        } else {
          NULL
        }
        plot_col <- if ("plotnumber" %in% env_fields) {
          "plotnumber"
        } else if ("PlotNumber" %in% env_fields) {
          "PlotNumber"
        } else {
          NULL
        }
    ...
    reorder_sibling <- function(direction = c("up", "down")) {
      ...
      if (swap_idx < 1 || swap_idx > nrow(siblings)) {
        return()
      }
    ...
    observeEvent(input$su_delete, {
      ...
      if (row_idx < 1 || row_idx > nrow(rv$su)) {
        return()
      }
    ...
    observeEvent(input$su_hot, {
      ...
      if (!all(c("PlotNumber", "SiteUnit") %in% names(new_df))) {
        return()
      }
    ...
    observeEvent(input$su_master_level, {
      if (!identical(rv$su_mode, "master")) {
        return()
      }
      rv$su <- load_su("master", master_level = input$su_master_level)
    })
