  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$publish_panel <- renderUI({
      if (!auth_is_authenticated(state) || !auth_user_has_permission(state, "publish_rds")) {
        return(card(card_header("Project Publishing"), card_body("Sign in with publish permissions to access this panel.")))
      }
      publish_panel_ui(ns)
    })
    ...
    output$download_panel <- renderUI({
      if (!auth_is_authenticated(state) || !auth_user_has_permission(state, "view_download_logs")) {
        return(card(card_header("Download Log"), card_body("Sign in with download log permissions to access this panel.")))
      }
    ...
    observe({
      ...
      if (is.null(input$publish_projects)) {
        return()
      }
      projects <- tryCatch(
        {
      ...
      if (nrow(projects) > 0) {
        updateSelectInput(session, "publish_projects", choices = setNames(projects$projectid, paste(projects$projectid, "-", projects$projecttitle)))
      }
    })
    ...
    observeEvent(input$publish_run, {
      if (!require_permission("publish_rds")) {
        return()
      }
      out_dir <- trimws(input$publish_out_dir %||% "")
      if (!nzchar(out_dir)) {
